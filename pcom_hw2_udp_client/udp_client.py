...
* random - continuously send random payloads from the list''')
    ...
    server_address = parser.add_argument_group(
        'Server Address & Port (required)')
    server_address.add_argument('server_ip', help='Server IP', type=ip_address)
    server_address.add_argument(
        'server_port', help='Server Port', type=unprivileged_port_type)
    ...
    source_address.add_argument('--source-port', type=unprivileged_port_type, default=0,
                                metavar=get_unprivileged_port_meta(
                                    'source-port'),
    ...
    load.add_argument(
        '--delay', help='Wait time (in ms) between two messages (default: 0)', type=int, default=0)
...
def send_message(sock, message, parsed_args):
    ...
    sent = sock.sendto(
        to_send, (str(parsed_args.server_ip), parsed_args.server_port))
    print('Sent ({}/{} bytes) << {} >>'.format(sent,
          len(to_send), message['description']))
...
def run_manual(sock, parsed_args):
    ...
    options = '\n'.join(['{}. {}'.format(i, x['description'])
                        for i, x in enumerate(parsed_args.input_file)])
    input_ask = 'Input a int ({} - {}): '.format(0,
                                                 len(parsed_args.input_file) - 1)
