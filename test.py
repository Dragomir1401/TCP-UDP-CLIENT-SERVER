...
class Topic:
    ...
    def generate_topics():
        ...
        ret.append(
            Topic("abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwx", "INT", "10"))
        ...
        ret.append(Topic("huge_string", "STRING",
...
class Process:
    ...
    def start(self):
        ...
        try:
            if self.cwd == "":
                self.proc = Popen(
                    self.command, universal_newlines=True, stdin=PIPE, stdout=PIPE, stderr=PIPE)
            else:
                self.proc = Popen(self.command, universal_newlines=True,
                                  stdin=PIPE, stdout=PIPE, stderr=PIPE, cwd=self.cwd)
            self.started = True
...
def run_udp_client(mode=True, type="0"):
    ...
    if mode:
        udpcl = Process(["python3", "udp_client.py",
                        ip, port], udp_client_path)
        udpcl.start()
        for i in range(19):
            outudp = udpcl.get_output_timeout(1)
        udpcl.finish()
    else:
        udpcl = Process(["python3", "udp_client.py", "--mode",
                        "manual", ip, port], udp_client_path)
        udpcl.start()
        sleep(1)
...
def check_subscriber_output(c, client_id, target):
    ...
    if target not in outc:
        print("Error: C" + client_id +
              " output should contain [" + target + "], is actually [" + outc.rstrip() + "]")
        return False
...
def run_test_compile():
    ...
    fail_test("compile")
    print("Compiling")
    exit_if_condition(not make_target("server"),
                      "Error: server could not be built")
    exit_if_condition(not make_target("subscriber"),
                      "Error: subscriber could not be built")
    pass_test("compile")
...
def run_test_quick_flow(c1, topics):
    ...
    for i in range(6):
        for topic in topics:
            success = check_subscriber_output(
                c1, "1", topic.print()) and success
